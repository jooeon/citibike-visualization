...
export class ChronologicalRenderer {
  ...
  render(): void {
    ...
    this.activeTrips.forEach(activeTrip => {
      if (activeTrip.trip) {
        // Robust check: Only render trips from valid, selected stations
        if (activeTrip.trip.startStationIndex === undefined ||
            activeTrip.trip.startStationIndex < 0 ||
            activeTrip.trip.startStationIndex >= this.stations.length ||
            !this.selectedStationIndices.has(activeTrip.trip.startStationIndex)) {
          return; // Skip this trip
        }
